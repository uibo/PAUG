...
def Get_soldoutpost (item, date, file_num) :
    ...
    for page_num in range(1,100):
        ...
        if content:
            with open(f"soldout_URL/soldout_url_naracafe{file_num}.txt", 'a') as f:
                for post in content:
...
def Crawling_until_today(year, month, item):
    ...
    while year != 2024 or month != 6:
        ...
        try:
            # 1일부터 15일
            print(date[0])
            Get_soldoutpost(item, date[0], file_num)
            # 16일부터 말일
            print(date[1])
            Get_soldoutpost(item, date[1], file_num)
