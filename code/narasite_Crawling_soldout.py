...
def Get_soldout_post(item, min_price, max_price):
    ...
    for page_num in range(1,126):
        ...
        try:
            print(page_num)
            path_url = f"/search/{item}?page={page_num}&saleYn=SALE_Y&sort=RECENT_SORT&minPrice={min_price}&maxPrice={max_price}&productFilterType=APP"
            ...
            with open(f"soldout_url/soldout_url_{min_price}-{max_price}.txt", 'a') as f:
                for post in soup:
                    soldout_post = post.find("div", string = "판매완료")
                    if soldout_post:
                        soldout_post_url = root_url + soldout_post.find_parent('a')['href']
                        f.write(soldout_post_url)
                        f.write('\n')
            error_count = 0
